...
/// 2. `[writable]` dogbone: [DogboneMetadata] 
...
/// - short_description: [String] 
	pub fn process_mint(
		...
		let dogbone_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (dogbone_pubkey, dogbone_bump) = Pubkey::find_program_address(
			&[b"dogbone", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *dogbone_info.key != dogbone_pubkey {
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), dogbone_info.clone()],
			&[&[b"dogbone", mint_info.key.as_ref(), &[dogbone_bump]]],
		)?;
		...
		if dogbone_info.data_len() != DogboneMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let dogbone = &mut AccountPDA::new(
			&dogbone_info,
			try_from_slice_unchecked::<DogboneMetadata>(&dogbone_info.data.borrow()).unwrap(),
			dogbone_bump,
		);
		...
		dogbone.data.serialize(&mut &mut dogbone_info.data.borrow_mut()[..])?;

...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let primary_color: String = Default::default();
		let accent_color: String = Default::default();
		...
		let (dogbone_pubkey, _) = Pubkey::find_program_address(
			&[b"dogbone", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(dogbone_pubkey, false),

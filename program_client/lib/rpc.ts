 ...
 * 2. `[writable]` dogbone: {@link DogboneMetadata} 
 ...
 *
 * Data:
 * - primary_color: {@link string} 
 * - accent_color: {@link string} 
 * - rarity: {@link string} 
...
export const mint = (args: MintArgs): TransactionInstruction => {
    ...
    const [dogbonePubkey] = pda.deriveDogboneMetadataPDA({
        mint: args.mint,
    }, _programId);
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            {pubkey: dogbonePubkey, isSigner: false, isWritable: true},
...
export const getDogboneMetadata = async (
    publicKey: PublicKey,
    commitmentOrConfig: Commitment | GetAccountInfoConfig | undefined = "processed"
): Promise<T.DogboneMetadata | undefined> => {
    ...
    return T.decodeDogboneMetadata(deserialize(T.DogboneMetadataSchema, buffer.data) as Record<string, unknown>);
}
